...
async function getAllFlights() {
  ...
  const rows = await db.all(`
    SELECT f.id, a1.city as origin, a2.city as destination, f.duration
    FROM flight f
...
async function getPassengers(flightId) {
  ...
  const passengers = await db.all(
    ...
    FROM passenger p
    JOIN flight ON flight.id = p.flight_id
...
async function testQuery() {
  ...
  //   console.log(await getFlight(1));
  console.log(await getPassengers(1));
}
